...
PYTHONSCRIPT="/home/ubuntu/git/CDIS_GEUVADIS_analysis/run_docker.beta.py";
...
### From Sean 1-21-16
# failures=0 ;for ((i=1;i<=100;i++)); do curl -I -s http://ftp.sra.ebi.ac.uk/vol1/fastq/ERR188/ERR188094/ERR188094_2.fastq.gz 2>&1 >/dev/null; status_cmd=$?; if [[ ${status_cmd} -gt 0 ]]; then failures=$(( ${failures}+1 )); fi; echo "${i} ${status_cmd} ${failures}"; done

# This will try to grab the headers from the url of your wget request and just display the exit code of curl. Every time I have run this for loop I have almost a 50% failure rate from the remote server.
