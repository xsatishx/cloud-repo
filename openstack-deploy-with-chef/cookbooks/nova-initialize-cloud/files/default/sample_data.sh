...
keystone user-role-add --user $GLANCE_USER \
                       --role $ADMIN_ROLE \
...
keystone user-role-add --user $NOVA_USER \
                       --role $ADMIN_ROLE \
                       --tenant_id $SERVICE_TENANT
...
keystone user-role-add --user $EC2_USER \
                       --role $ADMIN_ROLE \
                       --tenant_id $SERVICE_TENANT
...
keystone user-role-add --user $SWIFT_USER \
                       --role $ADMIN_ROLE \
                       --tenant_id $SERVICE_TENANT
...
#RESULT=$(keystone ec2-credentials-create --tenant_id=$SERVICE_TENANT --user=$ADMIN_USER)
...
credential_file="/root/admin_auth"
ADMIN_USERNAME="admin"
OS_AUTH_URL="http://${CONTROLLER_PUBLIC_ADDRESS}:5000/v2.0/"
EC2_URL="http://${CONTROLLER_PUBLIC_ADDRESS}:8773/services/Cloud"

    echo "export OS_TENANT_NAME=$ADMIN_USERNAME" >> $credential_file
    echo "export OS_USERNAME=$ADMIN_USERNAME" >> $credential_file
    echo "export OS_PASSWORD=$ADMIN_PASSWORD" >> $credential_file
    echo "export OS_AUTH_URL=${OS_AUTH_URL}" >> $credential_file
    ...
    CREDS=$(keystone ec2-credentials-create --tenant_id=$ADMIN_TENANT --user=$ADMIN_USER)
