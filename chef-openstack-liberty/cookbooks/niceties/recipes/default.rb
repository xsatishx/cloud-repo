...
`dpkg --get-selections|grep syslog`.scan(/^([\S]+)[\s]+([\S]+)/).each { |a, b| $packages[b] << a }
...
if $packages['install'].include? 'rsyslog'
  ...
  puts '#### We would do something here, if this recipie supported syslog-ng' if $debug
else
  puts '####' if $debug
  puts '####' if $debug
  puts '####' if $debug
  puts '####' if $debug
  puts '#### Is no syslog daemon of any sort installed?  Look at this node!' if $debug
  puts '####' if $debug
  puts '####' if $debug
  puts '####' if $debug
end
...
nicities.each do |target, source|
  template target do
    ...
    if service.include? target
      ...
      puts "Working on #{servicename}" if $debug
