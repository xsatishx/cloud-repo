...
bonding_packages = %w(ifenslave ethtool)
...
# Fine, just install this everywhere
...
if data.include?('bonds') && not(data['bonds'].empty?)
  puts ''
  puts ''
  puts 'Founding bonding config'
  puts ''
  puts ''
  bonding_packages.each do |bpkg|
    package bpkg do
      action :install
    end
  end
  ...
  data['bonds'].each do |bond|
    ...
    # slaves
    data[bond]['slaves'].each do |slave|
  ...
  puts ''
  puts ''
  if data.include?('bridges') && not(data['bridges'].empty?)
  ...
  if data.include? 'interfaces'
    ...
    data['interfaces'].each do |interface|
      puts "looking at #{interface} which should also be in (#{data.class})data{#{data.keys.join(", ")}}"
      puts data.keys.collect{|i| i+"=>"+data[i].to_json}.join(",")
      ...
      template "/etc/network/interfaces.d/#{interface.gsub('.', '_')}" do
        ...
        #if(data[interface].is_a?NilClass)
  ...
  if data.include? 'interfaces'
    ...
    data['interfaces'].each do |interface|
      if data.include?('bridges') && data['bridges'].include?(interface)
        next
      end
      template "/etc/network/interfaces.d/#{interface.gsub('.', '_')}" do
        mode '664'
